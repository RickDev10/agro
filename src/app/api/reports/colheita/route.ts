...
// GET - Relatório de colheita (PROTEGIDO)
async function handleGet(request: AuthenticatedRequest) {
  try {
    console.log('🔍 GET /api/reports/colheita - SEGURO - Usuário:', request.userEmail)
    ...
    console.log('✅ Relatório de colheita gerado (SEGURO):', data?.length || 0, 'registros')
    ...
    console.error('❌ Erro ao buscar dados de colheita:', error)
    return NextResponse.json(
      { success: false, error: 'Erro ao gerar relatório de colheita' },
      { status: 500 }
    )
  }
}
